  ...
  const handleCapture = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        formData.append('file', file);  // Changed to 'file' to match FastAPI's default

        // Send to backend
        const response = await fetch('https://mobileconsultant.wha7.com/upload-image/', {
          method: 'POST',
          body: formData,
        });
        ...
        const transformedResults = {
          scores: [
            { label: "Style", value: analysisResults.Evaluation_style },
            { label: "Fit", value: analysisResults.Evaluation_fit },
            { label: "Color", value: analysisResults.Evaluation_color },
            { label: "Matching", value: analysisResults.Evaluation_matching },
            { label: "Trendiness", value: analysisResults.Evaluation_trendiness },
            { label: "Overall", value: analysisResults.Evaluation_overall_look }
          ],
          summary: analysisResults.Giving,
          description: analysisResults.Review,
          learnMoreUrl: "https://wha7.com/app"
        };
